...
create policy "lks bill class amounts admin write" on public.lks_bill_class_amounts for all using (
  public.is_admin()
) with check (
  public.is_admin()
);

comment on table public.lks_bill_class_amounts is
  'Nominal khusus tagihan LKS per kelas. Jika tidak ada baris untuk kelas tertentu, sistem memakai lks_bills.total_amount.';
