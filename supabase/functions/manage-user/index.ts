...
Deno.serve(async (req) => {
  ...
  if (!supabaseUrl || !anonKey || !serviceRoleKey) {
    return jsonResponse({
      error: 'Supabase function env vars are not configured. Set SUPABASE_URL, SUPABASE_ANON_KEY or SUPABASE_PUBLISHABLE_KEY, and SUPABASE_SERVICE_ROLE_KEY or SUPABASE_SECRET_KEY.',
    }, 500);
  }
  ...
  if (callerError || callerProfile?.role !== 'admin' || callerProfile?.is_active !== true) {
  ...
  try {
    if (action === 'list') {
      ...
      const profiles = (plainProfiles || []).map((profile) => ({
        ...profile,
        assigned_class: profile.assigned_class_id ? classById.get(profile.assigned_class_id) || null : null,
      }));
    ...
    if (action === 'create') {
      ...
      return jsonResponse({ user: data.user, profile: {
        id: data.user.id,
        full_name: payload.full_name,
        email: payload.email,
        role: payload.role,
    ...
    if (action === 'update') {
      if (!payload.id) return jsonResponse({ error: 'ID user wajib diisi' }, 400);
    ...
    if (action === 'delete') {
      if (!payload.id) return jsonResponse({ error: 'ID user wajib diisi' }, 400);
      if (payload.id === authData.user.id) return jsonResponse({ error: 'Admin tidak bisa menghapus akun sendiri' }, 400);
    ...
    if (action === 'set-active') {
      ...
      if (payload.id === authData.user.id && payload.is_active === false) {
        return jsonResponse({ error: 'Admin tidak bisa menonaktifkan akun sendiri' }, 400);
      }
    ...
    return jsonResponse({ error: error.message || 'Failed to manage user' }, 400);
  }
});
