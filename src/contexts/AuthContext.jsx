...
export function AuthProvider({ children }) {
  ...
  async function loadProfile(userId) {
    ...
    try {
      ...
      setProfile(data);
      setProfileError(data ? '' : 'Profil user belum ada. Hubungi admin atau jalankan seed/admin profile.');
      return data;
    } catch (error) {
      setProfile(null);
  ...
  const value = useMemo(
    () => ({ session, user: session?.user ?? null, profile, profileError, loading, signIn, signOut, refreshProfile: () => loadProfile(session?.user?.id) }),
