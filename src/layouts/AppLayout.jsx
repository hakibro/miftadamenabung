...
const navByRole = {
  admin: [
    ...
    { to: '/admin/students', label: 'Siswa', icon: GraduationCap, mobile: true },
  ...
  bendahara: [
    { to: '/bendahara/dashboard', label: 'Dashboard', icon: Home, mobile: true },
    { to: '/bendahara/reports', label: 'Laporan', icon: BarChart3, mobile: true },
    { to: '/bendahara/reports/classes', label: 'Kelas', icon: BookOpen, mobile: true },
    { to: '/bendahara/reports/students', label: 'Siswa', icon: GraduationCap, mobile: true },
  ],
  walas: [
...
function LogoMark({ logoUrl, appName }) {
  if (logoUrl) {
    return <img src={logoUrl} alt={appName} className="h-10 w-10 rounded-lg border border-slate-200 object-cover" />;
  }
          ...
          <div className="flex h-16 items-center justify-between gap-3 px-4 sm:h-18 lg:px-6">
            <div className="flex min-w-0 items-center gap-3 lg:hidden">
              ...
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold text-slate-950">{schoolName}</p>
                <p className="truncate text-xs text-slate-500">{pageTitle}</p>
              </div>
            </div>
            <div className="hidden min-w-0 lg:block">
              <p className="truncate text-sm text-slate-500">{pageTitle}</p>
              <h1 className="truncate text-xl font-semibold text-slate-950">Halo, {profile?.full_name || 'Pengguna'}</h1>
            </div>
            <button
              onClick={signOut}
