...
import { BookOpen, CalendarRange, HandCoins, ImageUp, Settings as SettingsIcon, X } from 'lucide-react';
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <button onClick={() => setActiveCategory('charges')} className="rounded-[24px] bg-white p-5 text-left shadow-soft">
          <HandCoins className="mb-3 text-brand-700" />
          <p className="font-bold text-slate-950">Tagihan</p>
          <p className="text-sm text-slate-500">Buku, kegiatan, dan tagihan lain.</p>
        </button>
      ...
      {activeCategory ? (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/45 p-4 backdrop-blur-sm" onMouseDown={() => setActiveCategory(null)}>
          <div className="mx-auto max-w-6xl rounded-[28px] bg-[#f7f1ff] p-4 shadow-soft" onMouseDown={(event) => event.stopPropagation()}>
            <div className="mb-4 flex items-center justify-between gap-3">
              <h2 className="text-lg font-bold text-slate-950">{activeCategory === 'app' ? 'Settings Aplikasi' : activeCategory === 'charges' ? 'Settings Tagihan' : activeCategory === 'initial' ? 'Setting Data Awal' : 'Setting Tahun Ajaran Baru'}</h2>
      ...
      {activeCategory === 'charges' ? <ChargesPage mode="settings" embedded /> : null}

      {activeCategory === 'initial' ? <div className="space-y-5">
        <div className="rounded-2xl bg-white p-4 text-sm text-slate-600 shadow-soft">
          Setup awal aplikasi: buat tahun ajaran aktif, import siswa dengan format Excel saat ini, kelas akan otomatis dibuat jika data import berisi tingkat dan nama kelas. Setelah itu assign wali kelas secara manual.
        </div>
        <div className="rounded-2xl bg-white p-4 shadow-soft">
          <h3 className="mb-3 font-bold text-slate-950">1. Buat Tahun Ajaran</h3>
          <PeriodsPage />
        </div>
        <div className="rounded-2xl bg-white p-4 shadow-soft">
          <h3 className="mb-3 font-bold text-slate-950">2. Import Siswa</h3>
          <p className="mb-4 text-sm text-slate-500">Gunakan kolom name, nis, gender, grade/tingkat, class_name, note. Jika kelas belum ada, sistem membuatnya otomatis di tahun ajaran aktif.</p>
          <ImportPage />
        </div>
        <div className="rounded-2xl bg-white p-4 shadow-soft">
          <h3 className="mb-3 font-bold text-slate-950">3. Kelas dan Assign Wali Kelas</h3>
          <p className="mb-4 text-sm text-slate-500">Kelas opsional dibuat manual jika belum dibuat dari import siswa. Wali kelas dapat dipilih manual per rombel.</p>
          <ClassesPage />
        </div>
      </div> : null}
      ...
      {activeCategory === 'new-year' ? <div className="space-y-5">
        <div className="rounded-2xl bg-white p-4 text-sm text-slate-600 shadow-soft">
          Buat tahun ajaran baru, rombel otomatis, proses siswa naik kelas, import siswa baru kelas 1, lalu assign wali kelas manual.
        </div>
        <PromotionPage />
      </div> : null}
