...
export default function StudentsPage() {
	...
	async function load() {
		const [students, classRows, periodRows] = await Promise.all([
			listStudents(),
			listClassOptions(),
			listPeriodOptions(),
		]);
		...
		setFilters((current) => ({
			...current,
			periodId:
				current.periodId || periodRows.find((item) => item.is_active)?.id || "",
		}));
	}
	...
	const filteredRows = useMemo(() => {
		...
		return rows.filter((student) => {
			...
			const matchesPeriod =
				!filters.periodId || studentClass?.period_id === filters.periodId;
			const matchesGrade =
				!filters.grade || Number(studentClass?.grade) === Number(filters.grade);
			const matchesClass =
				!filters.classId ||
				student.current_class_id === filters.classId ||
				studentClass?.id === filters.classId;
	...
	const classOptions = useMemo(
		() =>
			classes.filter((item) => {
				const matchesPeriod =
					!filters.periodId || item.period_id === filters.periodId;
				const matchesGrade =
					!filters.grade || Number(item.grade) === Number(filters.grade);
	...
	async function bulkSetActive(isActive) {
		...
		await Promise.all(
			targets.map((student) =>
				saveStudent({ id: student.id, is_active: isActive }),
			),
		);
		setToast(
			isActive ? "Siswa terpilih diaktifkan" : "Siswa terpilih dinonaktifkan",
		);
		setSelectedIds([]);
		await load();
	}
	...
	async function bulkMoveClass() {
		...
		await Promise.all(
			targets.map((student) =>
				saveStudent({ id: student.id, current_class_id: bulkClassId }),
			),
		);
		...
		<div className="space-y-5">
			...
			<div className="space-y-3">
				<div className="grid gap-3 rounded-[22px] border border-white/80 bg-white p-4 shadow-soft md:grid-cols-[1fr_190px_160px_220px]">
					...
					<select
						...
						value={filters.grade}
						onChange={(e) =>
							setFilters({ ...filters, grade: e.target.value, classId: "" })
						}>
					...
					<div className="flex flex-col gap-2 md:col-span-4 md:flex-row md:items-center md:justify-between">
						...
						<button
							...
							className="rounded-xl bg-brand-600 px-4 py-2 font-semibold text-white shadow-glow">
							Tambah Siswa
						</button>
					</div>
					{selectedIds.length ? (
						<div className="grid gap-2 rounded-2xl bg-brand-50 p-3 md:col-span-4 md:grid-cols-[1fr_auto_auto_auto] md:items-center">
							...
							<button
								className="text-sm font-semibold text-slate-500 md:text-right"
				...
				<DataTable
					...
					columns={[
						...
						{ key: "gender", label: "JK" },
						{
							key: "period",
							label: "Tahun Ajaran",
							render: (row) => row.current_class?.periods?.name || "-",
						},
						{
							key: "grade",
							label: "Tingkat",
							render: (row) =>
								row.current_class?.grade
									? `Tingkat ${row.current_class.grade}`
									: "-",
						},
						{
							key: "class",
							label: "Kelas",
							render: (row) => row.current_class?.name,
						},
						{
							key: "is_active",
							label: "Status",
					...
					actions={(row) => (
						<div className="space-x-2">
							...
							<button className="text-red-600" onClick={() => remove(row.id)}>
								Hapus
							</button>
						</div>
					)}
			...
			{showForm ? (
				<div
					...
					<div
						className="mx-auto max-w-lg rounded-[24px] bg-white p-4 shadow-soft"
						...
						<form onSubmit={submit} className="space-y-4">
							...
							<FormField label="Kelas">
								<select
									...
									onChange={(e) =>
										setForm({ ...form, current_class_id: e.target.value })
									}
									required>
									<option value="">Pilih kelas</option>
									{formClassOptions.map((item) => (
										<option key={item.id} value={item.id}>
											{item.name} - {item.period?.name || "Tahun ajaran"}
										</option>
									))}
								</select>
							</FormField>
							...
							<button className="w-full rounded-md bg-brand-600 px-4 py-2 font-semibold text-white">
								Simpan
							</button>
						</form>
					</div>
