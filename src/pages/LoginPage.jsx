...
export default function LoginPage() {
  const { signIn, signOut, user, profile, profileError, loading } = useAuth();
  ...
  useEffect(() => {
    if (user && profile) navigate(from || roleHome(profile.role), { replace: true });
  }, [user, profile, from, navigate]);

  if (!loading && user && profile) return <Navigate to={from || roleHome(profile.role)} replace />;
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-100 px-4">
      <div className="w-full max-w-md rounded-lg border border-slate-200 bg-white p-6 shadow-soft">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold text-slate-900">Sistem Keuangan Kelas</h1>
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          {user && !profile && profileError ? (
            <div className="rounded-md bg-amber-50 p-3 text-sm text-amber-800">
              <p>{profileError}</p>
              <button type="button" className="mt-2 font-semibold text-amber-900" onClick={signOut}>
